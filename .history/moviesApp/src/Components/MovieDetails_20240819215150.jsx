...
export default function MovieDetails(){
    ...
    useEffect(()=>{
        ...
        axios.get(url,options).then(function(res){
            ...
            let dirSet = new Set(director)
            let prodSet = new Set(producer)

            for(let ele of crewObj){
                if(ele.job.includes("Director")){
                    dirSet.add(ele.original_name)
                    setDirector([dirSet])
                }

                if(ele.job.includes("Producer")){
                    prodSet.add(ele.original_name)
                    setProducer([...prodSet])
                }
            }
                ...
                <div className="py-2">
                    ...
                    <div className="flex mt-4">
                        {director.map((name,index)=>{
                            return <p className="mr-4" key={index}>{name}</p>
                        })}
                        {producer.map((name,index)=>{
                            return <p className="mr-4" key={index}>{name}</p>
                        })}
                    </div>
                </div>
